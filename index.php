...
session_start();
$connect = mysqli_connect('localhost','root','','shopmedz');
...
//Remove item in cart 
    if(isset($_GET["action"]))
    {
    	if($_GET["action"] == "delete")
        {
            ...
            {
            	if($value["item_id"] == $_GET["Product_id"])
              	{
...
<html>  	
    ...
    <body>  
        ...
        <?php
        $qury = "SELECT * FROM final_dataset_e_commerce ORDER BY Product_id ASC";
                ...
                <div class="col-md-4">  
                    <form method="post" action="index.php?action=add&Product_id=<?php echo $row["id"];?>">  
                    	<div style="border:1px solid #333; background-color:#f1f1f1; border-radius:5px; padding:16px;" align="center">  
                    		...
                    		<h4 class="text-info"><?php echo $row['Product Title'];?></h4>  
                    		<h4 class="text-danger">$<?php echo $row['Price'];?></h4>  
                    		<input type="text" name="quantity" class="form-control" value="1" />
                    		<input type="hidden" name="hidden_name" value="<?php echo $row['Product Title'] ?>" />
                    		<input type="hidden" name="hidden_image" value="<?php echo $row['Image Urls'];?>">
